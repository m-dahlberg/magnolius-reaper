-- Note Leveling -- 32-bit float WAV writer, used only to build test fixtures.
--
-- A copy of the DeNoise script's dn/wav.lua. This script never writes audio --
-- it writes automation -- so the writer lives in test/ rather than in nl/. It
-- is here because verify_edit_in_reaper needs a take whose notes and levels
-- are known exactly, and the only way to get one is to synthesise it.
--
