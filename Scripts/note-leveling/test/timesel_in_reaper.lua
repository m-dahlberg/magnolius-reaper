...
local run_ok, run_err = pcall(function()
  ...
  -- No split here: NoteLeveling writes envelope points, not takes. `origin` is what carries
  -- the range: it is where frame 0 sits in PROJECT time. Left at item_pos, a ride measured
  -- over a selection would be written at the start of the item -- right shape, wrong place.
  near(Analyze.geometry(take).origin, POS, 1e-6, "with no range, frame 0 is the item start")
  near(geo.origin, SEL0, 1e-6, "with a range, frame 0 is where the selection starts")
  near(geo.origin - geo.item_pos, geo.t0, 1e-9, "origin and t0 agree")

  -- A clip the range does not touch reads nothing, rather than reading its own start for the
  -- range's length -- which produced "Item is too short to analyse" and stopped the whole run
  -- on a track whose other clips were simply elsewhere.
  local off = Analyze.geometry(take, { t0 = POS + len + 1, t1 = POS + len + 3, whole = false })
  near(off.acc_len, 0, 1e-9, "a non-overlapping range reads nothing at all")
  near(off.total_samples, 0, 0, "zero samples, not a whole item's worth")
