-- @noindex
-- Adaptive De-Click -- 32-bit float WAV writer.
--
-- ReaScript can read audio through an accessor but has no sink, so the
-- repaired result is written here and handed back to REAPER as a media source.
-- Float, always: the chain can overshoot 0 dBFS on transient-heavy material and
-- a float file simply carries it rather than clipping.
--
