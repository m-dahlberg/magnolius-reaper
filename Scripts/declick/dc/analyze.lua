-- @noindex
-- Adaptive De-Click -- stage 1: read the file and build the envelope cache.
--
-- The only stage that touches audio on the way in. It runs the kernel's
-- analysis task over the whole take once and leaves behind the per-band
-- step-peak envelopes in the kernel's heap. Everything after it -- detection,
-- the threshold, the displays -- works off those, which is why the panel can
-- respond to a slider without re-reading a single sample.
--
...
-- Anything that changes which samples we would read, or the shape of what we
-- build out of them. Detection parameters must NOT invalidate this -- being
-- able to move the threshold without re-reading the file is the whole design.
...
-- latter runs off the end of the accessor and captures the stretch twice. And
-- the rendered file, being item-time audio at the source rate, must go back on
-- a take with playrate 1, not with the original's playrate copied onto it.
...
-- `range` is an optional time-selection range in PROJECT seconds (see dc/timesel.lua). The
-- take accessor is anchored at 0 at the start of the ITEM, so a range beginning part way in is
-- read from `range.t0 - item_pos`; that subtraction happens once, here, and everything
-- downstream just uses t0 and acc_len.
function M.geometry(take, range)
  ...
  return {
    ...
    -- acc_len is the accessor's span, which is what everything downstream
    -- measures itself against. Named for what it is, so it cannot quietly be
    -- confused with the length of the source file again. With a time selection
    -- it is the span of the SELECTION, starting t0 into the take.
...
-- Coroutine body. Yields read requests, returns a result table or nil + message.
function M.run(take, cfg, k, range)
  local geo = M.geometry(take, range)
  local stepsz = Config.step_samples(cfg, geo.rate)
  if geo.total_samples < stepsz * 16 then
    return nil, "Item is too short to analyse at this step size"
  end
  ...
  local steps = k:get("ana_steps")
  if steps < 8 then
    return nil, "Analysis produced no steps -- is the item silent?"
  end
  return { geo = geo, steps = steps }
end
