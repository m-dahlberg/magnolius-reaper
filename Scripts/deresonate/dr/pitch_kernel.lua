-- @noindex
-- The pitch kernel: YIN and per-frame level, compiled from EEL.
--
-- Copied verbatim from `NoteLeveling script/nl/kernel.lua` apart from this
-- header and the .eel path. It is a copy rather than a shared module because
-- every script in this family is standalone -- the same reason each carries
-- its own `wav.lua`.
--
-- DeResonate uses only what feeds the harmonic mask: f0 and aperiodicity per
-- frame, plus the level the voicing gate needs. Note clustering is NOT copied;
-- the mask wants per-frame pitch, not semitone-labelled notes.
...
local function compile(ImGui, ctx, script_dir)
  local path = script_dir .. "dr/dsp/pitch.eel"
