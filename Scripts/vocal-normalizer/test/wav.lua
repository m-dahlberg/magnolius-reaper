-- Vocal Normalizer -- 32-bit float WAV writer, used only to build test
-- fixtures.
--
-- A copy of the Note Leveling script's test/wav.lua. This script never writes
-- audio -- it writes one volume number -- so the writer lives in test/ rather
-- than in vn/. It is here because the in-REAPER suites need takes whose
-- loudness is known exactly, and the only way to get one is to synthesise it.
--
